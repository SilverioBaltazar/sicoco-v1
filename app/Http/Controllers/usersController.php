    ...
    */
    ...
    public function actionLogin(loginuserRequest $request){
        ...
        if($existe->count()>=1){
            ...
            session(['userlog' => $request->usuario,'passlog' => $request->password,'usuario' => $usuario,'ip' => $ip, 'status' => $status, 'role' => $role_name, 'userkey' => $user_id ]);
            //dd('Usuario: '.$usuario.' - Rango: '.$rango.' - Estructura: '.$estructura.'- Dependencia: '.$dependencia.' - Nombre dependencia: '.$nombre_dependencia);
            toastr()->info($nombre,'Bienvenido ');
            return view('sicinar.menu.menuInicio',compact('usuario','userkey','nombre','ip','role'));
        }else{
    ...
    public function actionVerUser(){
        ...
        $roles      = regRolModel::select('ROL_ID','ROL_NAME','ROL_DESC')
                      ->get();
        $users      = User::join('COMB_USERS_ROLES'  ,'COMB_USERS_ROLES.USER_ID','=','COMB_USERS.USER_ID')
                          ->join('COMB_ROLES'        ,'COMB_ROLES.ROL_ID'       ,'=','COMB_USERS_ROLES.ROL_ID')
                        ->select('COMB_USERS.USER_ID','COMB_USERS.USER_NAME','COMB_USERS.USER_EMAIL',
                                 'COMB_ROLES.ROL_ID' ,'COMB_ROLES.ROL_NAME','COMB_ROLES.ROL_DESC',
                                 'COMB_USERS.USER_PASSWORD','COMB_USERS.STATUS')
                       ->orderBy('COMB_USERS.USER_ID'  ,'ASC')
                       ->orderBy('COMB_USERS.USER_NAME','ASC' )
                       ->paginate(30);        
        //dd($usuarios->all());
    ...
    public function actionEditarUser($id){
        ...
        $roles      = regUserRolModel::join('COMB_ROLES','COMB_ROLES.ROL_ID','=','COMB_USERS_ROLES.ROL_ID')
                      ...
                      ->get();
        $users      = User::join('COMB_USERS_ROLES','COMB_USERS_ROLES.USER_ID','=','COMB_USERS.USER_ID')
                      ->select(  'COMB_USERS_ROLES.ROL_ID',
                                 'COMB_USERS.USER_ID','COMB_USERS.USER_NAME','COMB_USERS.USER_EMAIL',
                                 'COMB_USERS.USER_PASSWORD','COMB_USERS.STATUS')
                      ->where('COMB_USERS.USER_ID',$id)
                      ->first();        
    ...
    public function actionActualizarUser(userRequest $request, $id){
        ...
        if($existe->count()>=1){
            ...
            //******** actualizar matriz usuario - rol - privilegios tabla pivote COMB_USERS_ROLES_PRIVILEGS *******
            $tot_privilegios = regPrivilegModel::max('PRIVILEG_ID');
            if($request->rol_id == '3'){   //user [1..3]
                for($i=1;$i<=3;$i++){
                    // asignar privilegios todos x el rol de user (user)
