...
use App\Http\Requests\recibo21Request;
...
class recibo21Controller extends Controller
{


    public function actionEditarRecibo21($id){
        ...
        return view('sicinar.recibo.editarRecibo21',compact('regperiodo','regmes','regdia','regquincena','regplaca','regmarca','regtipogasto','regtipooper','regrecibos','nombre','usuario'));
    }

    public function actionActualizarRecibo21(recibo21Request $request, $id){
        $nombre        = session()->get('userlog');
        $pass          = session()->get('passlog');
        if($nombre == NULL AND $pass == NULL){
            return view('sicinar.login.expirada');
        ...
        if($regrecibos->count() <= 0)
            ...
            $name2 =null;
            //dd($request->recibo_bfoto1);
            //   if(!empty($_PUT['recibo_rfoto1'])){
            if(isset($request->recibo_bfoto1)){
                if(!empty($request->recibo_bfoto1)){
                    //Comprobar  si el campo foto1 tiene un archivo asignado:
                    if($request->hasFile('recibo_bfoto1')){
                      $name2 = $id.'_'.$request->file('recibo_bfoto1')->getClientOriginalName(); 
                      //sube el archivo a la carpeta del servidor public/images/
                      $request->file('recibo_bfoto1')->move(public_path().'/images/', $name2);
                    }
                }
            }
            ...
            toastr()->success('Bitacora de rendimiento de combustible actualizado correctamente.','¡Ok!',['positionClass' => 'toast-bottom-right']);
